...
def process_pos_annotation(path, uid):
    '''
    This function loads an image, checks the orientation, reads nodule locations for
    this specific image from the luna annotations file, modifies the nodule locations to 
    take the image orientation into acount, and creates a new csv file specific to the input uid
    containing the rectified nodule locations
    Inputs:
        path: path to the image to be processed 
        uid: uid of the patient of interest
    '''

    annotations_path = os.path.join(settings.CA_RAW_DATA_DIR,"Annotations","annotations.csv") 
    ...
    patient_imgs = utils.load_patient_images(uid, "_i.png") #volumes are stores in a certain way, and have to be loaded accordingly
    ...
    save_path = os.path.join(settings.CA_LABELS_DIR,uid,uid+"_annos_pos.csv")
...
def process_luna_candidates(path, uid):
    '''
    This function loads an image, checks the orientation, reads nodule locations for
    this specific image from the luna false positives file, ensures that the LUNA false 
    positives do not overlap with any LIDC positives for this volume, modifies the candidate 
    locations to take the image orientation into acount, and creates a new csv file specific 
    to the input uid containing the rectified nodule locations
    Inputs:
        path: path to the image to be processed 
        uid: uid of the patient of interest
    '''

    annotations_path = os.path.join(settings.CA_Labels_DIR,uid,uid+"_annos_pos_lidc.csv") 
    ...
    annotations_path = os.path.join(settings.CA_RAW_DATA_DIR,"Annotations","candidates_V2.csv")
    ...
    save_path = os.path.join(settings.CA_LABELS_DIR,uid,uid+"_candidates_luna.csv")
    df_candidates.to_csv(save_path, index=False)

def process_auto_candidates(path, uid, sample_count=1000, candidate_type="white"):
    '''
    This function uses the segmented lung images to generate random candidate locations 
    far enough from any positive nodules
    Inputs:
        path: path to the image to be processed 
        uid: uid of the patient of interest
        sample_count: number of desired samples
        candidate_type: 
    '''    
    annotations_path = os.path.join(settings.CA_Labels_DIR,uid,uid+"_annos_pos_lidc.csv") 
    df_pos_annos = pandas.read_csv(annotations_path)
    img_dir = os.path.join(settings.CA_LUNA_IMG_DIR,uid) + "/"
    ...
    df_candidates = pandas.DataFrame(candidate_list, columns=["anno_index", "coord_x", "coord_y", "coord_z", "diameter", "malscore"])
    save_path = os.path.join(settings.CA_LABELS_DIR,uid,uid+"_candidates_"+candidate_type + ".csv")
    df_candidates.to_csv(save_path, index=False)
...
def main():
    process_annotations(mode=0)
    process_annotations(mode=1)
    process_annotations(mode=2)
